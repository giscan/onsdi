...
RUN mkdir -p /usr/src/onsdi

WORKDIR /usr/src/onsdi
...
RUN chmod +x /usr/src/onsdi/tasks.py \
    && chmod +x /usr/src/onsdi/entrypoint.sh

# app-specific requirements
RUN pip install --upgrade --no-cache-dir --src /usr/src -r requirements.txt
RUN pip install --upgrade -e .

ENTRYPOINT ["/usr/src/onsdi/entrypoint.sh"]
